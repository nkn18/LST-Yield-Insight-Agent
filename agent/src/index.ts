...
async function startAgent(
    ...
    try {
        ...
        await fetchdataService.runFetchData();
        
        // Schedule periodic fetch data runs every 10 minutes
        const fetchDataIntervalMinutes = parseInt(process.env.FETCH_DATA_INTERVAL_MINUTES || "10", 10);
        const fetchDataIntervalMs = fetchDataIntervalMinutes * 60 * 1000;
        
        setInterval(() => {
            elizaLogger.info(`Running scheduled fetchdata (every ${fetchDataIntervalMinutes} minutes)`);
            fetchdataService.runFetchData().catch(err => {
                elizaLogger.error("Error in scheduled fetchdata run:", err);
            });
        }, fetchDataIntervalMs);
...
/**
 * Restarts an agent on a schedule
 * @param runtime The agent runtime to restart
 * @param directClient The direct client instance
 * @param intervalMinutes How often to restart the agent (in minutes)
 */
async function scheduleAgentRestart(
    ...
    setInterval(async () => {
        try {
            elizaLogger.info(`Scheduled restart for agent ${runtime.character.name} (${runtime.agentId})`);
            
            // Get the current character configuration
            const character = runtime.character;
            
            // Properly shut down all clients first
            if (runtime.clients && runtime.clients.length > 0) {
                elizaLogger.info(`Shutting down ${runtime.clients.length} clients for agent ${runtime.agentId}`);
                for (const client of runtime.clients) {
                    try {
                        if (client.stop && typeof client.stop === 'function') {
                            elizaLogger.debug(`Stopping client ${client.name}`);
                            await client.stop();
                        }
                    } catch (clientError) {
                        elizaLogger.error(`Error stopping client ${client.name}:`, clientError);
                    }
                }
            }
            ...
            directClient.unregisterAgent(runtime.agentId);
            ...
            // Start a new agent with the same character
            const newRuntime = await startAgent(character, directClient);
            elizaLogger.success(`Successfully restarted agent ${character.name} (${newRuntime.agentId})`);
        } catch (error) {
            elizaLogger.error(`Error during scheduled restart of agent ${runtime.character.name}:`, error);
        }
    }, intervalMs);
    
    elizaLogger.info(`Scheduled agent ${runtime.character.name} to restart every ${intervalMinutes} minutes`);
}
...
const startAgents = async () => {
    ...
    try {
        for (const character of characters) {
            ...
            // Set up automatic restart if enabled
            const autoRestartEnabled = parseBooleanFromText(process.env.AUTO_RESTART_AGENTS || "false");
            const restartInterval = parseInt(process.env.AGENT_RESTART_MINUTES || "10", 10);
            
            if (autoRestartEnabled) {
                await scheduleAgentRestart(runtime, directClient, restartInterval);
            }
        }
    } catch (error) {
